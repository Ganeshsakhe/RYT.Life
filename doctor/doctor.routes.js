...
router.get('/profile', (req, res) => {
    misc.checkUser(req, res).then(
        (user) => {
            doctorController.getProfile(user).then(
                (data) => {
                    console.log(data)
                    let user = data
...
router.put('/updateDoctor', (req, res) => {
    ...
    if (req.user && req.body.updatedData) {
        ...
        doctorController.updateUser(query, dataToBeUpdated, function (err, docs) {
            if (err) {
...
router.post('/createTimeSlots', (req, res) => {
    ...
    {
        let query = {
            "doctorId": req.user[0]._id
        }
        appointmentController.getDoctorTimeSlots(query, (err, docs) => {
            if (err) {
                console.log(err)
                ...
                //update timeslots
                appointmentController.updateDoctorTimeslots(query, req.body, function (err, docs) {
                    if (err) 
                    {
                        resp.errorResponse(res);
                    } else if (docs)
            ...
            } else 
            {
                ...
                appointmentController.createSlots(query, req.body, (err, docs) => {
                    ...
                    if (err) {
                        ...
                        resp.noRecordsFound(res, 'Unable to create doctors timeslots');
                    }
                });


                //resp.noRecordsFound(res, 'Unable to fetch doctors timeslots');
            }
        })
    }
})

router.get('/getDoctorTimeslots', (req, res) => {
    console.log("inside getDoctorTimeslots");
    if (req.user) {
        console.log("doctor", req.user)
        console.log("doctorid", req.user[0]._id)
        const query = {};
        query['doctorId'] = req.user[0]._id;
        appointmentController.getDoctorTimeSlots(query, (err, docs) => {
            if (err) {
                console.log(err)
                resp.errorResponse(res, err, 501, 'Error While fetching timeslots');
...
router.put('/updateDoctorTimeslots', (req, res) => {
    console.log("inside update doctor")
    if (req.user && req.body.updatedData) {
        console.log("inside update route", req.user)
        console.log("inside update body", req.body.updatedData)
        ...
        query['doctorId'] = req.user[0]._id;
        const dataToBeUpdated = req.body.updatedData
        appointmentController.updateDoctorTimeslots(query, dataToBeUpdated, function (err, docs) {
            if (err) {
                resp.errorResponse(res);
            } else if (docs) {
                resp.successPostResponse(res, docs, 'User Data updated successfully');
...
router.put('/cancelTimeslots', (req, res) => {
    console.log("inside cancelTimeslots doctor")
    if (req.user && req.body.updatedData) {
        console.log("inside update route", req.user)
        console.log("inside update body", req.body.updatedData)
        console.log("doctorid", req.user[0]._id)
...
router.get('/upcomingAppointment', (req, res) => {
    ...
    if (req.user) {
        ...
        var c;
        doctorController.getBookedSlots(query, function (err, docs) {
            if (err) {
                resp.errorResponse(res);
                ...
                doctorController.getBookedSlots_employee(query, function (err, employee_docs) {
                    if (err) {
                        resp.errorResponse(res);
...
router.get('/EmployeeappointmentFlag', (req, res) => {
    ...
    if (req.user) {
        ...
        query['doctorId'] = req.user[0]._id;
        //query["status"] = "appointment confirmed";
        doctorController.getBookedSlots_employee(query, function (err, docs) {
            if (err) {
                resp.errorResponse(res);
            } else if (docs.length > 0) {
                console.log("docs", docs);
        ...
        if (req.query.role == 'user') {
           ...
           try { appointmentController.updateBooking(query, dataToBeUpdated, (err, docs) => {
                ...
                if (err) {
                    ...
                    console.log("doctor docs", docs)
                    const query1 = {};
                    query1['doctorId'] = docs.doctorId._id;
                    ...
                    appointmentController.cancelTimeslots(query1, dataToBeUpdated, function (err, docs1) {
                        ...
                        if (err) {
                            ...
                            doctorController.appointmentCancelEmail(docs, function (err, docs2) {
                                if (err) {
                            ...
                            doctorController.appointmentCancelEmailDoctor(docs, function (err, docs3) {
                                if (err) {
                                    console.log("sendgrid err", err);
                                    //resp.errorResponse(res,err,501,"Internal Server Error, Please Try Again Later");
                                } else if (docs3) {
                ...
                if (err) {
                    ...
                    appointmentController.cancelTimeslots(query1, dataToBeUpdated, function (err, docs1) {
                        if (err) {
                            ...
                            doctorController.appointmentCancelEmail_employee(docs, function (err, docs2) {
                                if (err) {
                                    console.log("sendgrid err", err);
                                    //resp.errorResponse(res,err,501,"Internal Server Error, Please Try Again Later");
                                } else if (docs2) {
                            ...
                            doctorController.appointmentCancelEmailDoctor_employee(docs, function (err, docs3) {
                                if (err) {
                                    console.log("sendgrid err", err);
                                    //resp.errorResponse(res,err,501,"Internal Server Error, Please Try Again Later");
                                } else if (docs3) {
...
router.get('/logout', function (req, res) {
    if (req.headers && req.headers.authorization) {
        ...
        doctorController.logout(headers[1], (err, docs) => {
            if (err) {
                resp.errorResponse(res);
            }
...
router.get('/getAllclient', (req, res) => {
    ...
    if (req.user) {
        ...
        doctorController.getAllclient(query, function (err, docs) {
            if (err) {
                resp.errorResponse(res);
            } else if (docs) {
                console.log("docs", docs)
                ...
                console.log(user_result);
                doctorController.getAllEmployeeclient(query, function (err, employee_docs) {
                    if (err) {
                        resp.errorResponse(res);
...
router.get('/getclientbyId', (req, res) => {
    ...
    if (req.user) {
        ...
        if (req.query.role == 'user') {
            ...
            doctorController.getAllEmployeeclient(query, function (err, docs) {
                if (err) {
                    resp.errorResponse(res);
                } else if (docs.length > 0) {
...
router.get('/getAllfinance', (req, res) => {
    ...
    if (req.user) {
        ...
        // query['$or'] = [{ "status": "appointment confirmed" }, { "status": "appointment cancelled" }, { "status": "appointment closed" }];
        query["booking_status"] = "paid";
        doctorController.getAllfinance(query, function (err, docs) {
            if (err) {
                resp.errorResponse(res);
            } else if (docs.length > 0) {
                console.log("docs", docs)
    ...
    if (req.body && req.query) {
        appointmentController.updateBooking(query, dataToBeUpdated, (err, docs) => {
            if (err) {
                ...
                resp.noRecordsFound(res, 'Unable to updated doctors notes');
            }
        })
    }
})
